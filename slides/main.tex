...
\begin{frame}{A Brief History of ICA Identifiability (1/2)}
  ...
  \begin{alertblock}{Nonlinear ICA: Provably Non-Identifiable
    ...
    \begin{itemize}
      \item Infinitely many $(\mathbf{f}',\mathbf{s}')$ pairs are consistent with any observed $p(\mathbf{x})$
      \item Any smooth bijection of $\mathbf{s}$ yields an equally valid explanation
    \end{itemize}
    \[
...
\begin{frame}{A Brief History of ICA Identifiability (2/2): Auxiliary Variables}
  ...
  \begin{itemize}
    ...
    \item E.g.\ time-segment index, experimental condition — not always available
  \end{itemize}
...
\begin{frame}{Log MI: Optimal GAN Discriminator}
  ...
  \bigskip
  \begin{theorem}[Goodfellow et al., 2014 — Optimal discriminator]
...
\begin{frame}{Multiple Noisy Views}
  ...
  \begin{theorem}[Theorem 8 — informal]
    Under sufficient regularity conditions (bounded noise variance, non-degeneracy of
    $\mathbf{g}$), exact recovery of $\mathbf{s}$ up to affine ambiguity is possible in
    the limit $N\to\infty$.
  \end{theorem}
